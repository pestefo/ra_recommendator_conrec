...
def extract_extended_tags_for_users():
    ...
    for user in tqdm(users):
        ...
        for question in questions:
            try:
                user_tags[user] = user_tags[user].union(
                    tag_extractor.full_extended_tag_ids_for(question))
            except sqlite3.OperationalError:
                print ("Operantional ERROR on question: {} \t user: {}".format(
                    question, user))
